...
// GetWithPagination returns paginated tasks and total count.
func (r *TaskRepository) GetWithPagination(ownerID string, page, limit int, sortBy, order string) ([]models.Task, int64, error) {
	...
	query := r.db.Model(&models.Task{}).Preload("Owner")
	if ownerID != "" {
		query = query.Where("owner_id = ?", ownerID)
	}
	...
	if err := query.Order(orderClause).Limit(limit).Offset(offset).Find(&tasks).Error; err != nil {
		return nil, 0, err
	}
