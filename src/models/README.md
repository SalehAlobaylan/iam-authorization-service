...
This folder contains the core domain models for the Wahb IAM/auth service.
They are shaped to match the SQL migrations under `database-migrations/migrations`
and the current IAM service contracts.
...
- **User model**
  ...
  - Added associations to `Tokens` and `Roles`, plus `TableName()` and `BeforeCreate` for UUIDs.
...
- **Important fields**:
  - `ID uuid.UUID`: primary key.
  - `Resource string`: logical resource name (e.g. `"user"`, `"profile"`, `"iam"`).
...
- **Claims helper**:
  - `PermissionClaim` groups allowed actions by resource for embedding in JWT
    access tokens: `{ "resource": "user", "actions": ["read", "delete"] }`.
